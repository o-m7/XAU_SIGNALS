#!/usr/bin/env python3
"""
COMPREHENSIVE MODEL EVALUATION - OUT-OF-SAMPLE (2025)

Tests every model on TRUE out-of-sample data (2025).
Ensures NO lookahead bias by testing on period AFTER training.

Models trained on:
- Most models: 2020-2023 data
- Sniper models: 2024 data

Test on: 2025 ONLY (never seen by any model)
"""
...
RESULTS_DIR = PROJECT_ROOT / "research_results" / "all_models_evaluation_OOS"
...
# CMF+MACD v4 baseline (from 2024 results - need to re-test on 2025)
BASELINE_WR = 0.616
BASELINE_PF = 1.60
...
def main():
    print("=" * 80)
    print("OUT-OF-SAMPLE MODEL EVALUATION - 2025 DATA ONLY")
    print("=" * 80)
    print(f"\n⚠️  TESTING ON TRUE OUT-OF-SAMPLE DATA")
    print(f"   Most models trained on: 2020-2023")
    print(f"   Sniper models trained on: 2024")
    print(f"   Test period: {TEST_START} to {TEST_END} (2025 - NEVER SEEN)")
    print()
    ...
    print(f"  Test period (2025): {len(df_test):,} rows")
    ...
    # Evaluate all models
    print(f"\n[4] Evaluating all models on 2025 data (this may take 5-10 minutes)...")
    results = []
    ...
    # Save full results
    df_results.to_csv(RESULTS_DIR / "all_models_ranked_2025_OOS.csv", index=False)

    # Print top 10
    print(f"\n" + "=" * 120)
    print("TOP 10 MODELS (Ranked by Score) - 2025 OUT-OF-SAMPLE")
    print("=" * 120)
    ...
    print(f"\n" + "=" * 120)
    print("🏆 BEST MODEL (2025 OUT-OF-SAMPLE)")
    print("=" * 120)
    ...
    print(f"  Profit/Trade:   {best['avg_return_after_costs_bps']:+.2f} bps")
    ...
    # Category breakdown
    print(f"\n" + "=" * 120)
    print("PERFORMANCE BY CATEGORY (2025 OUT-OF-SAMPLE)")
    print("=" * 120)
    ...
    print(f"\n\nResults saved to:")
    print(f"  - {RESULTS_DIR / 'all_models_ranked_2025_OOS.csv'}")

    # Count profitable models
    profitable_count = (df_results['profitable'] == True).sum()
    print(f"\n📊 Summary:")
    print(f"  Profitable models: {profitable_count}/{len(df_results)}")
    print(f"  Best WR: {df_results['win_rate'].max()*100:.1f}%")
    print(f"  Best Profit/Trade: {df_results['avg_return_after_costs_bps'].max():+.1f} bps")
    print()
