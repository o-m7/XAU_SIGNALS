#!/usr/bin/env python3
"""
Standalone Training Script for Model 6 (Order Flow / Microstructure)
...
MODEL_OUTPUT_PATH = MODEL_OUTPUT_DIR / "model6_orderflow.joblib"
...
def build_orderflow_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Spread features
    if 'spread_pct' not in df.columns and all(c in df.columns for c in ['bid', 'ask']):
        print("  Building spread features...")
        df['spread'] = df['ask'] - df['bid']
        df['spread_pct'] = df['spread'] / df['mid'] * 100
    elif 'spread_pct' not in df.columns and 'high' in df.columns and 'low' in df.columns:
        # Estimate spread from high-low
        df['spread_pct'] = (df['high'] - df['low']) / df['close'] * 100 * 0.1  # Rough estimate
    ...
    # Price momentum (micro)
    if 'micro_momentum' not in df.columns and 'close' in df.columns:
        print("  Building micro momentum...")
        df['micro_momentum'] = df['close'].pct_change(5) * 100
        df['micro_momentum_smooth'] = df['micro_momentum'].rolling(3).mean()
...
def main():
    print("=" * 80)
    print("MODEL 6 (ORDER FLOW) - STANDALONE TRAINING")
    print("=" * 80)
    print(f"\nTimestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"Train Period: {TRAIN_START} to {TRAIN_END}")
    print(f"Validation Period: {VAL_START} to {VAL_END}")
    print(f"\nStrategy: Order Flow / Microstructure Analysis")
    ...
    # Clean training data
    train_clean = train_df.dropna(subset=[label_col] + feature_cols[:10])
